...
const WomanCard = ({ handleClick, productType }) => {
  ...
  const handleData = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${baseUrl}/api/women`, {
        headers: {
      ...
      setError('Failed to fetch womens data');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!findGadget) {
    return <div>No women\'s product found with ID: {id}</div>;
  }
  ...
  return (
    ...
    <div className="container">
      ...
      <div className="img-container">
        <img src={findGadget.image} alt={findGadget.model || 'Women\'s Product'} />
      </div>
