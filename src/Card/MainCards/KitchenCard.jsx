...
const KitchenCard = ({ handleClick, productType }) => {
 ...
 const handleData = async () => {
    try {
      ...
      setKitchenData(Array.isArray(res.data) ? res.data : res.data.data || []);
    } catch (err) {
  ...
  if (!findGadget) {
    return <div>No kitchen item found with ID: {id}</div>;
  }
  ...
  return (
    <div className="container">
      ...
      <div className="img-container">
        <img src={findGadget.image} alt={findGadget.model || 'Kitchen Item'} />
      </div>
